...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     * 注意：**不要**再读 useSessions() 的 `current` 字段 —— 0.1.6-alpha.2 的 SessionListState
     * 只有 ids / byId / phase（dsh-api-session-controller 的 sessions/service.d.ts），没有 current，
     * 旧代码读它恒为 null，于是「会话投影 = 权威」这条路径从来没生效过（issue #5）。
     */
