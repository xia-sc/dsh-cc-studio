  ...
  factory: (require) => {
    ...
    function createStore(ctx){
      ...
      function saveSettings(){
        try{ localStorage.setItem("dsh-cc-studio-settings", JSON.stringify({ poll: !!state.pollEnabled, autoValidate: !!state.autoValidate, autoSaveOnImport: !!state.autoSaveOnImport })); }catch(e){}
      }
      function setPollEnabled(v){ set({pollEnabled: !!v}); saveSettings(); }
          ...
          inp.onchange=function(){
            ...
            reader.onload=function(){
              try{
                ...
                set({currentLibraryId:null, lastMsg:"已导入："+(obj.data.name||"未命名")+" · "+(obj.data.character_book.entries.length||0)+" lore（导入后可在侧边栏 ★ 保存）"});
    ...
    function Workshop(props){
      ...
      if(s.cur===0){
        main=h("div",null,
          h("div",{style:S.card},
              h("h3",{style:{margin:"0 0 8px", fontSize:13}}, "本地草稿搜索"),
              h("div",{style:{fontSize:12, color:"var(--dsw-alias-label-secondary)", marginBottom:8}}, "输入关键词实时过滤右侧“已存角色”侧栏，点“载入”直接覆盖当前草稿；标签芯片可一键筛选。"),
              h("div",{style:{display:"flex", gap:6}},
                h("input",{placeholder:"搜索 名称 / 标签 / 简介…", style:{flex:1, background:"var(--dsw-alias-bg-base)", border:"1px solid var(--dsw-alias-border-l1)", borderRadius:8, color:"var(--dsw-alias-label-primary)", padding:"8px 10px", fontSize:12, boxSizing:"border-box", maxWidth:"100%"}, value:s.libraryQuery, onChange:function(e){ store.setLibraryQuery(e.target.value); }, onKeyDown:function(e){ if(e.key==="Enter"){ store.refreshLibrary(); }}}),
                h("button",{style:Object.assign({}, S.btn, {padding:"6px 12px", fontSize:12, flexShrink:0}), onClick:function(){ store.refreshLibrary(); }}, "刷新"),
                h("button",{style:Object.assign({}, S.btnGhost, {border:"1px solid var(--dsw-alias-border-l1)", padding:"6px 10px", fontSize:12}), onClick:function(){ store.setLibraryQuery(""); }}, "清空")
              ),
            h("div",{style:{display:"flex", flexWrap:"wrap", gap:6, marginTop:8}},
              ["全部","雨城","感官系","赛博","治愈","废土","微克苏鲁"].map(function(tag){
                ...
                return h("span",{key:tag, style:Object.assign({}, S.chip, active?S.chipOn:{}), onClick:function(){ store.setLibraryQuery(tag==="全部"?"":tag); }}, tag);
              })
            ),
            (function(){
              ...
              if(list.length===0) return h("div",{style:{fontSize:12, color:"var(--dsw-alias-label-secondary)", textAlign:"center", padding:"16px 0"}}, s.library.length===0?"暂无已存草稿，去右侧侧栏“★ 保存当前”":"无匹配，试试其他关键词");
              return h("div",{style:{marginTop:10, display:"flex", flexDirection:"column", gap:8}},
                list.slice(0,3).map(function(entry){
                  return h("div",{key:"search-"+entry.id, style:{display:"flex", alignItems:"center", justifyContent:"space-between", gap:8, background:"var(--dsw-alias-bg-layer-1)", border:"1px solid var(--dsw-alias-border-l1)", borderRadius:8, padding:"8px 10px"}},
                    ...
                    h("button",{style:Object.assign({}, S.btn, {padding:"4px 10px", fontSize:12, flexShrink:0}), onClick:function(){ store.loadFromLibrary(entry.id); }}, "载入")
                  );
                }),
                list.length>3 ? h("div",{style:{fontSize:11, color:"var(--dsw-alias-label-secondary)", textAlign:"center"}}, "还有 "+(list.length-3)+" 个，详情看右侧侧栏 →") : null
              );
            })()
          ),
          h("div",{style:S.card},
        ...
        return h("div",{style:S.library},
          ...
          h("div",{style:{padding:"8px", borderBottom:"1px solid var(--dsw-alias-border-l1)", display:"flex", flexDirection:"column", gap:8, background:"var(--dsw-alias-bg-layer-1)"}},
            ...
            h("div",{style:{display:"flex", flexDirection:"column", gap:6}},
              h("button",{style:Object.assign({}, S.btn, {padding:"6px 10px", fontSize:12}), onClick:function(){ store.importJSON(); }}, "⬆ 导入本地 JSON（CCv3）"),
              h("div",{style:{fontSize:10, color:"var(--dsw-alias-label-secondary)", textAlign:"center"}}, "仅 JSON，PNG 二期 · 导入后 ★ 保存")
            ),
            h("div",{style:{display:"flex", gap:6}},
    ...
    function SettingsView(props){
      ...
      return h("div",{style:{padding:"16px 20px", maxWidth:720}},
        ...
        h("div",{style:{marginTop:12, background:"var(--dsw-alias-bg-layer-1)", border:"1px solid var(--dsw-alias-border-l1)", borderRadius:12, padding:12}},
          h("h3",{style:{margin:"0 0 8px", fontSize:13}},"工坊默认行为（本地保存）"),
          ToggleRow("CC 模式心跳 4s 同步", "关闭后 CC 模式下不再每 4s 拉取草稿，胶囊/预览需手动进工坊才同步。可显著减少网络请求。", pollEnabled, function(v){ store.setPollEnabled(v); }),
          ToggleRow("导入后自动校验", "导入 JSON 后自动调 validate 刷新右上角 通过/未通过 徽标。", autoValidate, function(v){ store.setAutoValidate(v); }),
          ToggleRow("导入后自动 ★ 保存", "导入后自动落盘到 ~/.dsh/cc-library（新建 ID），省去再点保存。", autoSaveOnImport, function(v){ store.setAutoSaveOnImport(v); }),
          h("div",{style:{fontSize:11, color:"var(--dsw-alias-label-secondary)", marginTop:8}}, "存储键 localStorage:dsh-cc-studio-settings，切预设/会话间共享。")
        ),
        h("div",{style:{marginTop:12, background:"var(--dsw-alias-bg-layer-1)", border:"1px solid var(--dsw-alias-border-l1)", borderRadius:12, padding:12}},
          h("h3",{style:{margin:"0 0 8px", fontSize:13}},"使用提示"),
          h("div",{style:{fontSize:12, color:"var(--dsw-alias-label-secondary)", lineHeight:1.6}}, "1. 切换 CC 模式 → 2. 让 LLM 按 6 步走（角色→五维→世界书≥5→问候语→validate）→ 3. 在工坊校验/导出或侧边栏保存。")
        )
      );
    }
    ...
    function apply(ctx){
      ...
      ctx.effect(function(){
        ...
        var timer = setInterval(function(){
          ...
          if(!s.isCcMode || !s.pollEnabled) return;
