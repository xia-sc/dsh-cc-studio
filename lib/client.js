...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var zh={
      ...
      "hint.capsule":"输入 /cc 唤出工坊（胶囊平时隐藏，避免突兀）；风格标签已支持自定义输入，回车添加",
    ...
    function makeDraft(){
      return {
        ...
        data:{
          ...
          creator_notes:"雨城示例草稿（融合形态）", creator_notes_multilingual:{ en:"Fusion prototype draft" }, source:[], assets:[{type:"icon", uri:"ccdefault:", name:"main", ext:"png"}],
    ...
    function createStore(ctx){
      var draft=makeDraft();
      var state={ draft: draft, cur:0, panelOpen:false, sideExpanded:false, triggered:false, isCcMode:false, customTag:"", idea:"雨城里的记忆典当行老板娘，能把记忆做成香薰蜡烛", tags:["雨城","感官系"], world:{ timeline:"2127 雨城独立后，记忆被列为可交易软资产", factions:"烛坊/雨梯管理局/盗忆人", geo:"环形雨城，内环干燥外环常雨", power:"抽忆术需自愿+嗅觉天赋", daily:"雨声电台、蜡烛夜市、记忆税" }, busy:null, valid:{valid:true,errors:[],warnings:[]}, lastMsg:null };
      ...
      function pushDraftToHost(){
        // fire-and-forget sync to host (for CC mode)
    ...
    function Capsule(props){
      ...
      return h("div",{style:S.dockWrap, "data-dsh-cc-studio":"dock-row"},
        ...
        h("button",{type:"button", style:{marginLeft:6, background:"transparent", border:"1px solid rgba(127,127,127,0.2)", borderRadius:999, padding:"0 8px", fontSize:11, lineHeight:"20px", color:"#9aa3b8", cursor:"pointer"}, onClick:function(){ store.dismiss(); }, title:"隐藏胶囊（可再输入 /cc 唤出）"}, "×")
      );
    }
      ...
      ctx.effect(function(){
        ...
        function poll(){
          ctx.connection.rpc.call("/dsh-cc-studio-rpc","cc_getDraft",{args:{}}).then(function(res){
            ...
            var isCc=!!res.value.isCc;
            store.setIsCcMode(isCc);
            if(isCc && !store.getSnapshot().triggered) store.trigger();
            try{
