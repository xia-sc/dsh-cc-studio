...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function SettingsView(props){
      ...
      // subscribe locale active
      var locSnapRef=React.useState(function(){ try{ return locale ? locale.getSnapshot() : null; }catch(e){ return null; } });
      var locSnap=locSnapRef[0], setLocSnap=locSnapRef[1];
      React.useEffect(function(){ if(!locale || !locale.subscribe) return; return locale.subscribe(function(){ try{ setLocSnap(locale.getSnapshot()); }catch(e){} }); }, [locale]);
      var activeLocale = locSnap && locSnap.active ? String(locSnap.active) : (function(){ try{ var s=locale&&locale.getLocale&&locale.getLocale(); return s&&s.active?String(s.active):null; }catch(e){return null;} })();
      // detect fallback: if browser zh then active zh, else en
      if(!activeLocale) activeLocale="zh";
      ...
      function LangButton(id, label){
        var on = activeLocale===id;
        return h("button",{onClick:function(){ try{ locale && locale.setLocale && locale.setLocale(id); }catch(e){} }, style:{flex:1, padding:"6px 10px", fontSize:12, fontWeight: on?600:400, borderRadius:8, border:"1px solid "+(on?"#7c5cff":"var(--dsw-alias-border-l1)"), background: on?"#7c5cff":"var(--dsw-alias-bg-layer-2)", color: on?"#fff":"var(--dsw-alias-label-primary)", cursor:"pointer"}}, label);
      }
      return h("div",{style:{padding:"16px 20px", maxWidth:720}},
        ...
        h("div",{style:{marginTop:12, background:"var(--dsw-alias-bg-layer-1)", border:"1px solid var(--dsw-alias-border-l1)", borderRadius:12, padding:12}},
          h("h3",{style:{margin:"0 0 8px", fontSize:13}}, t("settings.language.title")),
          h("div",{style:{fontSize:11, color:"var(--dsw-alias-label-secondary)", marginBottom:8, lineHeight:1.4}}, t("settings.language.desc")),
          h("div",{style:{display:"flex", gap:8}},
            LangButton("zh", t("settings.language.zh")),
            LangButton("en", t("settings.language.en"))
          ),
          h("div",{style:{fontSize:11, color:"var(--dsw-alias-label-secondary)", marginTop:8}}, "Active: "+activeLocale+" · "+(locale && locale.getLocale ? "via locale.setLocale" : "locale"))
        ),
        h("div",{style:{marginTop:12, background:"var(--dsw-alias-bg-layer-1)", border:"1px solid var(--dsw-alias-border-l1)", borderRadius:12, padding:12}},
