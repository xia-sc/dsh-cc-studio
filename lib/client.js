  ...
  factory: (require) => {
    ...
    function Capsule(props){
      ...
      React.useEffect(function(){
          ...
          try{
            if(currentId){
              ...
              if(sess){
                ...
                // sessions state may hold preset under other keys; scan session object string for cc
                try{ if(JSON.stringify(sess).toLowerCase().indexOf('"cc"')>=0) return true; }catch(e){}
              }
            }
          }catch(e){}
          try{
            // hero/全局：只要页面可见区域有“CC 模式”芯片即视为已选用 CC（覆盖无 currentId 的首选场景）
            var btns=document.querySelectorAll('button,[role="button"],span');
            for(var i=0;i<btns.length;i++){
              var b=btns[i];
              if(!b.textContent) continue;
              if(b.textContent.indexOf('CC 模式')>=0 && b.offsetParent!==null) return true;
            }
          }catch(e){}
          return false;
        }
        ...
        if(currentId){
          ...
          // 无会话但已选 CC（首页 hero 状态），异步确认主机侧并触发一次全局草稿拉取以预热
          store.checkIsCcMode(null);
        }
        // hero 场景 currentId 仍为 null，预设切换不会改变 currentId，故需轮询补位直到会话建立
        var iv=setInterval(function(){
          var nowCc=detectCc();
          if(nowCc && !store.getSnapshot().isCcMode) store.setIsCcMode(true);
          if(nowCc && currentId && store._pullDraft) store._pullDraft(currentId);
        }, 1200);
    ...
    function apply(ctx){
      ...
      ctx.effect(function(){
        ...
        // 全局轮询：1) hero 阶段 CC 预设无 currentId，dock 未挂载，需在此 root 效应中靠 DOM 兜底把 isCcMode 置 true 2) CC 模式下实时同步 LLM 的 cc_* Tool 写入
        var timer = setInterval(function(){
          var s = store.getSnapshot();
          if(!s.isCcMode && !s.triggered){
            try{
              var nodes=document.querySelectorAll('button,[role="button"],span');
              for(var i=0;i<nodes.length;i++){
                var n=nodes[i];
                if(n.textContent && n.textContent.indexOf('CC 模式')>=0 && n.offsetParent!==null){
            ...
            if(!store.getSnapshot().isCcMode && !store.getSnapshot().triggered) return;
          }
