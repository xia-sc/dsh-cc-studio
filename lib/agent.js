...
function workflowStatus(draft){
  ...
  let step = 1;
  if(!characterOk){
    step = 2; next = "下一步必须 cc_patch_character：补齐 name/description/personality/scenario（至少四项完整），不要跳去问候语";
  } else if(!worldOk){
    step = 3; next = "角色基础已齐，下一步必须 cc_patch_world：补全 5 维（timeline/factions/geo/power/daily 至少 3 项，autoLorebook=true），严禁跳去世界书/问候语";
  } else if(!loreOk){
    step = 4; next = "五维已齐，下一步必须 cc_add_lorebook_entries：补到至少 5 条（年表/势力/地理/力量/日常各 1，至少 1 条 constant），带 @@decorator + CBS，当前 "+loreCount+" 条";
  } else if(!greetingsOk){
    step = 5; next = "世界书已齐（"+loreCount+" 条），下一步必须 cc_patch_greetings：补 first_mes + alternate_greetings≥2 + group_only_greetings≥1";
  } else {
...
function apply(ctx){
  ctx.tools.register({
    name: "cc_get_card",
    description: "【工作流第1步·必调】查看当前 CC 角色卡的完整 JSON 草稿与工作流进度。CC 模式下 LLM 必须先调此工具，再按 露出进度→补齐 的顺序推进，严禁跳过五维/世界书。",
  ...
  ctx.tools.register({
    ...
    description: "【工作流第2步】写入/更新角色基础信息（name/nickname/description/personality/scenario/system_prompt/tags）。必须在 cc_get_card 之后、五维之前调用；写入后下一步必须是 cc_patch_world（≥3 维），不可直接跳到问候语。",
    parameters: {
      type:"object",
      properties:{
  ...
  ctx.tools.register({
    name: "cc_patch_world",
    description: "【工作流第3步·必调】写入 5 维世界观（timeline/factions/geo/power/daily，至少 3 项，autoLorebook=true）。严禁在角色基础未齐或世界书未齐时跳去问候语；调后下一步必须是 cc_add_lorebook_entries 补到≥5 条。",
    parameters: {
      type:"object",
      properties:{
  ...
  ctx.tools.register({
    ...
    description: "【工作流第4步·必调】添加 1-5 条 Lorebook 条目，补到至少 5 条（覆盖年表/势力/地理/力量/日常各 1，至少 1 条 constant），content 头部写 @@decorator。必须在五维之后、问候语之前调用。",
    parameters: {
      type:"object",
      properties:{
        entries:{
  ...
  ctx.tools.register({
    ...
    description: "【工作流第5步】写入开场与示例对话（first_mes / alternate_greetings≥2 / group_only_greetings≥1 必填 / mes_example）。必须在角色+五维+世界书(≥5 条)齐后才可调用，否则 cc_validate 会失败。",
    parameters: {
      type:"object",
      properties:{
  ...
  ctx.tools.register({
    name: "cc_validate",
    description: "【工作流第6步·收口必调】校验当前 CC 角色卡是否符合 SPEC_V3 + 工作流完整度（角色四件套、五维≥3、世界书≥5、问候语齐）。valid=false 时必须按 errors 继续补，valid=true 才可告诉用户去工坊“导出 JSON”或“★ 保存到侧边栏”（落盘 ~/.dsh/cc-library）。",
    parameters: { type:"object", properties:{}, additionalProperties:false },
    output: out({ type:"object", additionalProperties:true }),
    async execute(args, exec){
