...
- `0.2.22` Fix dsh `0.1.5-rc.1` boot crash `cannot get property "webServer" without inject`: from that version on, `connection.rpc.handle()` is unusable by an outside plugin (`HostConnectionService.rpc` closes over the connection plugin's own ctx, whose `inject` is only `["credentials"]` — its `webServer` lives in an inner `ctx.inject(["webServer"])` scope the owner fiber never resolves), so the host half now registers the `/dsh-cc-studio-rpc` prefix route on `webServer` itself and implements the same connection RPC wire protocol (including the `connection.requestRejection` fence and the `server-response` envelope); request bodies are read with `data/end/error` events instead of `for await` (async-iterating `IncomingMessage` throws in this runtime — even `client-connection`'s own `/api` frame is affected); adds `tests/rpc-channel.test.mjs` with 23 assertions; also fixes the `persona` row in `presets/cc` (`text` → required `prefix`, otherwise switching to CC mode fails with `invalid config: S.prefix missing required value`)
